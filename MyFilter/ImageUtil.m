...
static CGContextRef CreateRGBABitmapContext (CGImageRef image) {
    ...
    void *bitmapData;//内存空间的指针，该内存空间的大小等于图像使用RGBA通道所占用的字节数
    int bitmapByteCount;
    ...
    bitmapData = malloc(bitmapByteCount);//分配足够容纳图片字节数的空间
    context = CGBitmapContextCreate(bitmapData, pixelsWide, pixelsHigh, 8, bitmapBytesPerRow, colorSpace, kCGImageAlphaPremultipliedLast);//创建CoreGraphic的图形上下文，该上下文描述了bitmapData指向的内存空间需要绘制的图像的一些绘制参数
    CGColorSpaceRelease(colorSpace);//Core Foundation 中通过含有Create、Alloc方法名字创建的指针，需要使用CFRelease()函数释放
    return context;
    
}

static unsigned char *RequestImagePixelData(UIImage *image) {
    //返回一个指针，该指针指向一个数组，数组中的每四个元素都是一个图像的一个像素点的RGBA的数值（0-255），用无符号的char是因为它刚好取值范围（0-255）
